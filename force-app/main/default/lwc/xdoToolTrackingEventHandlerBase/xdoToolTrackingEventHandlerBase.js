...
export default class XdoToolTrackingEventHandlerBase extends NavigationMixin(
  LightningElement
) {
  _error;
  _componentName;
  ...
  get componentName() {
    ...
    return this._componentName && this._componentName != ""
      ? this._componentName
      : defaultName;
  }
  ...
  async registerTrackingHandlers(selectors = "") {
    // if (this.isCommunity) {
    this.callers = this.template.querySelectorAll(
      `${DEFAULT_TRACKING_SELECTOR}, ${selectors}`.replace(
        /(^\s*,)|(,\s*$)/g,
        ""
      )
    );
  ...
  normaliseDataset(dataset) {
    return Object.keys(dataset).reduce(
      (trackingData, key) => (
        (trackingData[
          `${TRACKING_PREFIX}${key.replace(TRACKING_PREFIX, "").slice(0, 1).toUpperCase() + key.replace(TRACKING_PREFIX, "").slice(1)}`
        ] = dataset[key]),
        trackingData
      ),
      {}
  ...
  trackEvent(dataset) {
    ...
    const e = new Event(normalDataset?.trackingDomEvent, {
      bubbles: true,
      cancelable: false
    });
  ...
  getPredicate(parameters, body) {
    return new Function(
      parameters,
      `console.log(args);  return ${body ? body : true};`
    );
  }
  ...
  handleEventDelegate(componentName) {
    return (event) => {
      ...
      const { trackingCondition: condition, trackingConditionArgs: args } =
        dataset;
      const trackingEvent = XdoToolTrackingEvent.createInstance(
        componentName,
        dataset
      );
      ...
      if (this.predicate(event)) {
        try {
          ...
          this.stagedEvent = window.setTimeout(
            this.publishTrackingEvent.bind(this, trackingEvent),
            trackingEvent.getDelay()
          );
        } catch (e) {
