...
export default class XdoToolMonitoringEvent extends XdoToolBaseEvent {
  constructor(componentName, event, data) {
    ...
    this.setType(TYPE_ENUM().MONITORING).setEvent(event).setLocalProps(data);
  }

  static ping(componentName, data) {
    return new XdoToolMonitoringEvent(
      componentName,
      EVENT_ENUM().MONITORING.PING,
      data
    );
  }
  ...
  setLocalProps(data) {
    this.properties.push(
      new XdoToolBaseEventProperty("system_up", data["trackingSystemUp"])
    );
    this.properties.push(
      new XdoToolBaseEventProperty(
        "response_time",
        data["trackingResponseTime"]
      )
    );
    return this;
  }
