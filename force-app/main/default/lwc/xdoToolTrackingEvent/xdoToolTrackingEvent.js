...
class XdoToolTrackingEvent {
  ...
  static getEventContext(trackingEvent) {
    ...
    for (let i = 0; i < typesLength; i++) {
      type =
        Object.keys(types[i][1]).filter(
          (event) => event === camelToSnake(trackingEvent).toUpperCase()
        ).length > 0
          ? types[i][0]
          : undefined;
  ...
  static createInstance(componentName, data) {
    ...
    try {
      return this.eventType[trackingType?.toUpperCase()][trackingEvent](
        componentName,
        data
      );
    } catch (e) {
      console.warn(
        `>>>>> Tracking event undefined for data-tracking-type: ${trackingType}, data-tracking-event: ${trackingEvent}`
      );
  ...
  static reportAvailableEvents(eventTypes) {
    ...
    Object.entries(eventTypes).forEach((entry) => {
      ...
      console.warn(
        `data-tracking-type : ${key.toLowerCase()} - data-tracking-event : ${JSON.stringify(this.getObjectMethods(value))}`
      );
    });
  }
}
